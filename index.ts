//import {halfCompareTapper, tapper} from "./tapper";
import {PERFORMANCE_METRIC_IDS, performanceMetrics} from "./hook";
import {AsyncResource} from 'async_hooks';
import {subtract} from "./actions/divide";

const a = '44324234234234365498678562423423423';
const k = '';

performanceMetrics();

const substructAsync = async () => {
  const hook = new AsyncResource(PERFORMANCE_METRIC_IDS.LOAD_DASHBOARD);
  const arr = [];
  for (let i = 0; i < 1000; i++) {
    arr.push({k, a});
  }
  await Promise.all(arr.map(({k: k1, a: a1}) => Promise.resolve(subtract(k1, a1))));
  
  hook.emitDestroy();
};

substructAsync();


/*
console.time('isSecondBigger');
console.log(isSecondBigger(k, a));
console.timeEnd('isSecondBigger');

console.time('subtract');

console.timeEnd('subtract');

console.time('subtract by mod');
console.log(subtractByModule(k, a));
console.timeEnd('subtract by mod');

console.time('subValues');
console.log(subValues(k, a));
console.timeEnd('subValues');*/

/*
console.time('tapper');
let i: number = 0;
const matrix: string[][] = [];
for (let y: string = k; parseInt(subtractByModule(y, k))<1; y = addByModule(y, '1')) {
  matrix.push([]);
  for(let x: number = 0; x <= 106; ++x) {
    matrix[i].push(halfCompareTapper(x.toString(), y) ? '*' : ' ');
  }
  i++;
}
console.timeEnd('tapper');

matrix.forEach(m => {
  console.log(m.reduce((acc, cur) => `${acc}${cur}`).split('').reverse().join(''));
});

console.log(`Matrix length: ${matrix.length}`);*/